...
final class AppDependencyContainer: ServiceContainer {
    ...
    /// Web URLs shared via the Share Extension that should be scraped through the
    /// web pipeline (same as `#URL` in chat input). Consumed once by `ChatDetailView`.
    var pendingIncomingWebURLs: [String] = []

    /// Incremented each time new web URLs arrive from the Share Extension.
    /// `ChatDetailView` observes this via `onChange` to trigger `processWebURL`.
